...
def prediction_page():
    ...
    historico_12_meses = camas_predichas[0]  # Diccionario con 12 meses anteriores + predicción
    mismo_mes_5_años = camas_predichas[1]  # Diccionario con mismo mes en 5 años diferentes
    temp_min, temp_max = camas_predichas[2]  # Temperaturas mínima y máxima
    confiabilidad = camas_predichas[3]       # Porcentaje de confiabilidad

    # Crear dos columnas (60% | 40%)
    col_pred, col_conf = st.columns([3, 2])

    with col_pred:
        # Tarjeta de predicción (texto original exacto)
        st.markdown(f"""
        <div style="text-align: center; margin: 10px 0; padding: 15px; border-radius: 10px; background-color: rgba(248, 249, 250, 0.5);">
            ...
            <h1 style="font-size: 4rem; color: #00CC96; margin: 20px 0;">{list(historico_12_meses.values())[-1]}</h1>
